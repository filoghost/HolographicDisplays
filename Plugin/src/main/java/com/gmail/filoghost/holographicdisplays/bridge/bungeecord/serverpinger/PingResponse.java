...
public class PingResponse {
	...
	public PingResponse(String jsonString, ServerAddress address) {
		...
		if (jsonString == null || jsonString.isEmpty()) {
			...
			ConsoleLogger.warning("Received empty Json response from IP \"" + address.toString() + "\"!");
			return;
		}
		...
		if (!(jsonObject instanceof JSONObject)) {
			motd = "Invalid ping response";
			ConsoleLogger.warning("Received invalid Json response from IP \"" + address.toString() + "\": " + jsonString);
			return;
		}

...
/**
 * This class is only used by the plugin itself. Do not attempt to use it. It
 * still implements the old API, but it's temporary.
 */
...
public class CraftHologram implements Hologram, com.gmail.filoghost.holograms.api.Hologram {
    ...
    public List<HologramLine> getHologramLines() {
        Builder<HologramLine> builder = ImmutableList.builder();
    ...
    @Override
    public void setLinesTouchHandler(com.gmail.filoghost.holographicdisplays.api.handler.TouchHandler handler) {
        if (size() > 0) {
            for (HologramLine line : getHologramLines()) {
                if (!(line instanceof TouchableLine))
                    continue;
                TouchableLine touchableLine = (TouchableLine) line;
                touchableLine.setTouchHandler(handler);
            }
        }
    }
    ...
    public String toString() {
        return "CraftHologram [world=" + world + ", x=" + x + ", y=" + y + ", z=" + z + ", lines=" + lines
                + ", deleted=" + deleted + "]";
    }
    ...
    /**
     * About: equals() and hashcode() Two holograms can never be equal. Even if
     * they have the same position and the same elements, they are still two
     * different objects. The equals and hashcode methods are not overriden, two
     * holograms are equal only if they have the same memory address.
     */

						...
						if (packet.getType() == PacketType.Play.Server.SPAWN_ENTITY_LIVING) {
							...
							for (int i = 0; i < dataWatcherValues.size(); i++) {
								if (dataWatcherValues.get(i).getIndex() == 2 && dataWatcherValues.get(i).getValue() != null) {
									//TODO DEEP CLONE dataWatcherValues.get(i).deepClone().getValue()
                                    //System.out.println(dataWatcherValues.get(i).getClass());
			...
			if (line.isSpawned()) {
				if (line instanceof CraftTextLine) {
					...
					if (itemLine.isSpawned()) {
						...
						metadata.add(new WrappedWatchableObject(new WrappedDataWatcher.WrappedDataWatcherObject(10), itemLine.getItemStack()));
						metadata.add(new WrappedWatchableObject(new WrappedDataWatcher.WrappedDataWatcherObject(1), (short) 300));
						metadata.add(new WrappedWatchableObject(new WrappedDataWatcher.WrappedDataWatcherObject(0), (byte) 0));
